...
def extract_metadata(file_path: Path) -> TrackMetadata | None:
    ...
    Returns None if the file format is unsupported or unreadable.
    """
    ...
    try:
        ...
        title = _get_title(audio, file_path)
...
def _get_title(audio: MutagenFile, file_path: Path) -> str:
    """Extract title tag, falling back to filename stem."""
